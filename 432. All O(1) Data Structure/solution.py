...
# Hashmap/Dict alone cant track frequence Well . 
# Create a Hybrid datastructure  : 
#     Doubley linked List  - Node ( freq , set ( keys )) - each node stores set of strings that have same frequency, Nodes are store in incremental order of frequency
#     Hashmap : { key , Node } 
# Hashmap helps to locate the node in Doubly LinkedList iin O(1) time. 
# Double LinkedList help to insert, Remove, GetMin( first node ), GetMax( Last node) in O(1) time